...
export default () => {
  ...
  const sendEmailConfirmation = React.useCallback(async (email) => {
    try {
  ...
  return (
    <Container>
      <Box mx={2}>
        ...
        {confirmEmail &&
          <>
            ...
            <P>Since you are signing in on a different device, please provide your email for confirmation.</P>
      ...
      <SlideInMessage handleClose={handleIncorrectEmailClose} open={incorrectEmailOpen} title="Incorrect Email">
      ...
      <SlideInMessage handleClose={handleErrorClose} open={errorOpen} title="Error Signing In">
        There was an error signing you in.  This can happen if the link is expired, or has already been used.
      </SlideInMessage>
    </Container >
  )
}
